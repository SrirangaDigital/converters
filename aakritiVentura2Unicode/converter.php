...
class Converter{
	...
	public function process ($fileName) {
		...
		$text = $this->ventura2Text($text);
		$text = $this->formHTML($text);

		$fileName = str_replace('/src/', '/xhtml/', $fileName);

		// file_put_contents($fileName . '.int.xhtml', $text);

		$text = $this->convert($text);

		file_put_contents($fileName . '.xhtml', $text);
	}
